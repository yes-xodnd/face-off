...
function PicResult({ state, methods }) {
  ...
  const ButtonDownload = () => (
    <button onClick={download} className="base-btn">저장하기</button>
  );

  const ButtonReEdit = () => (
    <button onClick={() => history.goBack()} className="base-btn">다시 수정하기</button>
  );

  const ButtonFriendRegister = () => (
    <button onClick={showModal} className="base-btn">아는 얼굴 등록하기</button>
  );
